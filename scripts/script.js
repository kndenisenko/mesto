...
// выбираем из начального массива все названия
const cardName = initialCards.map((cName) => {
  return cName.name;
})
//выбираем из начального массива все ссылки
const cardLink = initialCards.map((cLink) => {
  return cLink.link;
})
...
// Находим темплейт по ID и получаем его содержимое через .content и клонируем ноду
  ...
  const usersOnline = document.querySelector('.elements');                          // Находим место, куда будет вставлен темплейт

  userTemplate.querySelector('.element__image').src = card.link;                   //Вот тут магия. задаём картинку
  userTemplate.querySelector('.element__paragraph').textContent = card.name;        //Вот тут магия. задаём имя

  usersOnline.append(userTemplate);  // Появление карточки при загрузке страницы
}
